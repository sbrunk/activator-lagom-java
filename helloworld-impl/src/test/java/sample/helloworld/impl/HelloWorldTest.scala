...
class HelloWorldTest {
  @Test def testHelloWorld {
    val driver: PersistentEntityTestDriver[HelloCommand, HelloEvent, WorldState] = new PersistentEntityTestDriver[HelloCommand, HelloEvent, WorldState](HelloWorldTest.system, new HelloWorld, "world-1")
    ...
    val outcome2: PersistentEntityTestDriver.Outcome[HelloEvent, WorldState] = driver.run(new HelloCommand.UseGreetingMessage("Hi"), new HelloCommand.Hello("Bob", None))
